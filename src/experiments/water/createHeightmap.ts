...
export const createHeightmap = (renderer: THREE.WebGLRenderer) => {
  // Render once to texture
  const heightmapTexture = new THREE.WebGLRenderTarget(512, 512, {
    type: THREE.HalfFloatType,
  });
  const heightmapScene = new THREE.Scene();
  // Add a plane to render the heightmap
  const planeGeometry = new THREE.PlaneGeometry(100, 100, 1, 1);

  const plane = new THREE.Mesh(planeGeometry, new THREE.ShaderMaterial({
    fragmentShader: fragment,
    uniforms: {
      u_resolution: { value: new THREE.Vector2(512, 512) },
      u_seed: { value: Math.random() * 10000 }
    }
  }));
  plane.rotation.x = -Math.PI / 2;
  heightmapScene.add(plane);

  const cam = new THREE.OrthographicCamera(-1.2, 1.2, 1.2, -1.2, 0.1, 2);
  cam.position.y = 1;
  cam.lookAt(0, 0, 0);
  renderer.setRenderTarget(heightmapTexture);
  renderer.render(heightmapScene, cam);

  return heightmapTexture.texture;
}
